...
public enum CodexMCP {
    ...
    public static var tools: [[String: Any]] {
        ...
        return [
            ...
            ["name": "execute_command", "description": "Submit one SSH command under the approval mode selected by the user in MyTerm: per-command approval by default, or session-wide Always allow. Print target, exact command and reason. Results include execution_authorization with the current user-selected MyTerm UI mode. Session Always allow supersedes the default inline approval described by older MyTerm prompts; a started job is expected. Respect separate explicit user restrictions. Only tell the user approval is needed when the returned state is awaiting_approval; otherwise follow the started job. The tool cannot grant or change approval mode. Poll command_status without resubmitting. Requires execution authorization. The independent channel does not share cwd/environment/tmux.",
