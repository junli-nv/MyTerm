...
extension TerminalView {
    ...
    func mapColor (color: Attribute.Color, isFg: Bool, isBold: Bool, useBrightColors: Bool = true) -> TTColor
    {
        ...
        case .ansi256(let ansi):
            ...
            // Bright-bold only promotes the base eight colors. Explicit bright
            // and 256-color indices must never change when this option is off.
            midx = Int(ansi) + (useBrightColors && isBold && ansi < 8 ? 8 : 0)
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int) -> ViewLineInfo
    {
        var segments: [ViewLineSegment] = []
        #if os(macOS)
        let foregroundOverrides = rowForegroundProvider?(row, line, cols) ?? [:]
        #endif
        ...
        while true {
            ...
            var attr = ch.attribute
            #if os(macOS)
            if let color = foregroundOverrides[col] {
                attr = Attribute(fg: color, bg: attr.bg, style: attr.style,
                                 underlineStyle: attr.underlineStyle, underlineColor: attr.underlineColor)
            }
            #endif
